...
export default function SimuladorAhorroScreen({ navigation }) {
  ...
  const handleNext = () => {
    if (step === 1 && !meta) return showAlert('Atención', 'Ingresa tu meta de ahorro.');
    if (step === 2 && !aporte) return showAlert('Atención', 'Ingresa tu aporte mensual.');
    if (step === 3 && !tasa) return showAlert('Atención', 'Ingresa la tasa anual esperada.');
    if (step === 4 && !meses) return showAlert('Atención', 'Ingresa el plazo en meses.');
  ...
  const calcular = async () => {
    ...
    try {
      const response = await axios.post('https://twelve-laws-press.loca.lt/api/simulador/ahorro', {
  ...
  const renderStep = () => {
    ...
    switch(step) {
      case 1: question = "¿Cuál es tu meta económica total?"; placeholder = "Ej. 50000"; value = meta; setValue = setMeta; infoDesc = "La meta económica total es la cantidad final de dinero que deseas acumular. Por ejemplo, si estás ahorrando para comprar un auto que cuesta $200,000 MXN, esa es tu meta."; break;
      case 2: question = "¿Cuánto puedes aportar cada mes?"; placeholder = "Ej. 2000"; value = aporte; setValue = setAporte; infoDesc = "El aporte mensual es el dinero que destinarás de tu presupuesto cada mes hacia este ahorro específico. Te sugerimos que sea un monto realista que puedas cumplir consistentemente."; break;
      case 3: question = "¿Qué tasa de rendimiento anual esperas?"; placeholder = "Ej. 10"; value = tasa; setValue = setTasa; infoDesc = "La tasa de rendimiento anual es el porcentaje de ganancia que te pagará el instrumento donde guardes tu dinero (como CETES Directo, que suele dar ~11%). Si lo guardarás bajo el colchón, pon 0."; break;
      case 4: question = "¿En cuántos meses planeas lograrlo?"; placeholder = "Ej. 24"; value = meses; setValue = setMeses; infoDesc = "El plazo es la cantidad de meses que estarás aportando dinero. Si quieres lograr tu meta en 2 años, debes ingresar 24."; break;
    }
    ...
    return (
      <Animated.View style={[styles.resultContainer, { transform: [{ translateX: slideAnim }] }]}>
        <View style={styles.summaryCard}>
           ...
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Meta:</Text><Text style={styles.summaryValue}>${meta}</Text></View>
           ...
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Plazo:</Text><Text style={styles.summaryValue}>{meses} meses</Text></View>
        </View>

        <View style={styles.resultHeader}>
           <Text style={styles.resultTitle}>Proyección Final</Text>
           <Text style={styles.resultAmount}>${resultado.saldo_final.toLocaleString()}</Text>
           <Text style={styles.resultSubtitle}>Meta original: ${resultado.meta.toLocaleString()}</Text>
           <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 15 }}>
             <Ionicons name={resultado.alcanzado ? "checkmark-circle" : "alert-circle"} size={24} color={resultado.alcanzado ? colors.success : colors.warning} />
             <Text style={[styles.resultStatusText, { color: resultado.alcanzado ? colors.success : colors.warning, marginLeft: 8 }]}>
               {resultado.alcanzado ? '¡Meta Alcanzada!' : 'Faltó capital para la meta'}
             </Text>
           </View>
        </View>
