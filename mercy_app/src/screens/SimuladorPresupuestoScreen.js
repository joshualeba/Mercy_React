...
export default function SimuladorPresupuestoScreen({ navigation }) {
  ...
  const handleNext = () => {
    if (step === 1 && !ingresos) return showAlert('Atención', 'Ingresa tus ingresos totales.');
    if (step === 2 && !fijos) return showAlert('Atención', 'Ingresa tus gastos fijos.');
    if (step === 3 && !variables) return showAlert('Atención', 'Ingresa tus gastos variables.');
  ...
  const calcular = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.1.7:8000/api/simulador/presupuesto', {
  ...
  const renderStep = () => {
    ...
    switch(step) {
      case 1: question = "¿Cuáles son tus ingresos mensuales totales?"; placeholder = "Ej. 25000"; value = ingresos; setValue = setIngresos; infoDesc = "La suma de todos tus ingresos netos al mes (sueldo, rentas, negocios, etc) después de impuestos."; break;
      case 2: question = "¿Cuánto gastas en fijos (renta, servicios, etc)?"; placeholder = "Ej. 10000"; value = fijos; setValue = setFijos; infoDesc = "Los gastos fijos son los de supervivencia básica que debes pagar sí o sí cada mes: renta, hipoteca, luz, agua, despensa básica y seguros."; break;
      case 3: question = "¿Cuánto gastas en variables (salidas, antojos)?"; placeholder = "Ej. 5000"; value = variables; setValue = setVariables; infoDesc = "Gastos variables o discrecionales: salidas a cenar, cines, ropa, gustos, streaming y cosas que no son de estricta necesidad."; break;
    }
    ...
    return (
      <Animated.View style={[styles.resultContainer, { transform: [{ translateX: slideAnim }] }]}>
        <View style={styles.summaryCard}>
           ...
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Gastos Fijos:</Text><Text style={styles.summaryValue}>${fijos}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Gastos Variables:</Text><Text style={styles.summaryValue}>${variables}</Text></View>
        </View>

        <View style={styles.resultHeader}>
           <Text style={styles.resultTitle}>Balance Mensual</Text>
           <Text style={styles.resultAmount}>${resultado.balance?.toLocaleString()}</Text>
           <Text style={styles.resultSubtitle}>Estado actual: {resultado.estado}</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Simulador de Presupuesto</Text>
      </View>
