...
export default function SimuladorDeudaScreen({ navigation }) {
  ...
  const handleNext = () => {
    if (step === 1 && !saldoTotal) return showAlert('Atención', 'Ingresa el saldo de tu deuda.');
    if (step === 2 && !pagoMensual) return showAlert('Atención', 'Ingresa tu pago mensual.');
    if (step === 3 && !tasa) return showAlert('Atención', 'Ingresa la tasa de interés anual.');
  ...
  const calcular = async () => {
    ...
    try {
      const response = await axios.post('https://mercyreact.duckdns.org/api/simulador/deuda', {
  ...
  const renderStep = () => {
    ...
    switch(step) {
      case 1: question = "¿Cuál es el saldo total de tu deuda?"; placeholder = "Ej. 25000"; value = saldoTotal; setValue = setSaldoTotal; infoDesc = "La deuda total que tienes en la tarjeta de crédito o préstamo, sin contar los intereses futuros. Puedes ver esto en tu estado de cuenta."; break;
      case 2: question = "¿Cuánto puedes pagar mensualmente?"; placeholder = "Ej. 1500"; value = pagoMensual; setValue = setPagoMensual; infoDesc = "El pago que estás dispuesto a dar cada mes. Recuerda NUNCA dar solo el 'pago mínimo', ya que podrías tardar décadas en liquidarlo."; break;
      case 3: question = "¿Cuál es la tasa de interés anual (%)?"; placeholder = "Ej. 45"; value = tasa; setValue = setTasa; infoDesc = "La Tasa de Interés Anual Ordinaria o el CAT de tu tarjeta. Si es una tarjeta departamental clásica en México, suele rondar entre el 50% y el 70%."; break;
    }
    ...
    if (resultado.error) {
      ...
      <Animated.View style={[styles.resultContainer, { transform: [{ translateX: slideAnim }] }]}>
        <View style={styles.summaryCard}>
           ...
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Saldo de Deuda:</Text><Text style={styles.summaryValue}>${saldoTotal}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Tu Pago Mensual:</Text><Text style={styles.summaryValue}>${pagoMensual}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Tasa de Interés:</Text><Text style={styles.summaryValue}>{tasa}%</Text></View>
        </View>

        <View style={styles.resultHeader}>
           <Text style={styles.resultTitle}>Advertencia Crítica</Text>
           <Text style={[styles.resultAmount, { color: colors.danger, fontSize: 22, textAlign: 'center', marginTop: 10 }]}>{resultado.error}</Text>
        </View>
        ...
        <View style={{ marginTop: 5 }}>
           ...
           <View style={{ backgroundColor: colors.card, padding: 25, borderRadius: 20, borderTopLeftRadius: 0, borderWidth: 1, borderColor: colors.border, shadowColor: colors.primary, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4, marginLeft: 20 }}>
             {resultado.mensaje_ia ? resultado.mensaje_ia.split('\n').map((line, i) => {
               ...
               }) : <Text style={styles.aiText}>Calculando...</Text>}
           </View>
        </View>
    ...
    return (
      <Animated.View style={[styles.resultContainer, { transform: [{ translateX: slideAnim }] }]}>
        <View style={styles.summaryCard}>
           <Text style={styles.summaryTitle}>Tu Simulación</Text>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Saldo de Deuda:</Text><Text style={styles.summaryValue}>${saldoTotal}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Pago Mensual:</Text><Text style={styles.summaryValue}>${pagoMensual}</Text></View>
        ...
        <View style={styles.resultHeader}>
           <Text style={styles.resultTitle}>Meses para Pagar</Text>
           <Text style={styles.resultAmount}>{resultado.meses_para_pagar} meses</Text>
           <Text style={styles.resultSubtitle}>Total de intereses que pagarás: ${resultado.total_intereses?.toLocaleString()}</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Simulador de Deuda</Text>
      </View>
