...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ScrollView, Animated, Dimensions, Image, Modal, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function DiagnosticoScreen({ navigation }) {
  ...
  const handleNext = () => {
    if (step === 1 && !ingresos) return showAlert('Atención', 'Ingresa tus ingresos.');
    if (step === 2 && !gastos) return showAlert('Atención', 'Ingresa tus gastos.');
    if (step === 3 && !deuda) return showAlert('Atención', 'Ingresa tu deuda.');
    if (step === 4 && !ahorro) return showAlert('Atención', 'Ingresa tu ahorro actual.');
  ...
  const calcular = async () => {
    ...
    try {
      const response = await axios.post('https://mercyreact.duckdns.org/api/calcular_salud', {
  ...
  const styles = StyleSheet.create({
    ...
    resultAmount: { fontSize: 48, fontWeight: '900', marginBottom: 5 },
    resultSubtitle: { color: colors.text, fontSize: 16, textAlign: 'center', fontWeight: 'bold', marginTop: 10 },
  ...
  const renderStep = () => {
    ...
    switch(step) {
      case 1: 
        ...
        value = ingresos; 
        setValue = setIngresos; 
        infoDesc = "El dinero que recibes libre de impuestos cada mes por sueldo, negocios o rentas."; 
        break;
      case 2: 
        question = "¿Cuánto gastas en total al mes?"; 
        placeholder = "Ej. 18000"; 
        value = gastos; 
        setValue = setGastos; 
        infoDesc = "La suma de todos tus gastos: desde la renta y luz, hasta las salidas a comer y suscripciones."; 
        break;
      case 3: 
        question = "¿Cuál es el saldo total de tus deudas?"; 
        placeholder = "Ej. 50000"; 
        value = deuda; 
        setValue = setDeuda; 
        infoDesc = "Todo lo que debes actualmente en tarjetas, préstamos personales, hipoteca, etc."; 
        break;
      case 4: 
        ...
        value = ahorro; 
        setValue = setAhorro; 
        infoDesc = "Dinero disponible en cuentas de banco, inversiones líquidas o debajo del colchón."; 
        break;
    }
          ...
          <View style={styles.btnRow}>
             ...
             <TouchableOpacity style={styles.btnNext} onPress={handleNext} disabled={isCalculating}>
               {isCalculating ? (
                 ...
                 <Text style={styles.btnTextNext}>{step === 4 ? 'Evaluar Mi Salud' : 'Siguiente'}</Text>
               )}
             </TouchableOpacity>
          </View>
        </View>
  ...
  const renderResult = () => {
    ...
    const healthColor = getHealthColor(resultado.puntaje);
    
    return (
      <Animated.View style={[styles.resultContainer, { transform: [{ translateX: slideAnim }] }]}>
        <View style={styles.summaryCard}>
           <Text style={styles.summaryTitle}>Tus Números</Text>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Ingresos:</Text><Text style={styles.summaryValue}>${ingresos}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Gastos Totales:</Text><Text style={styles.summaryValue}>${gastos}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Deuda Total:</Text><Text style={styles.summaryValue}>${deuda}</Text></View>
           <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Ahorro Total:</Text><Text style={styles.summaryValue}>${ahorro}</Text></View>
        </View>

        <View style={styles.resultHeader}>
           <Text style={styles.resultTitle}>Calificación General</Text>
           <Text style={[styles.resultAmount, { color: healthColor }]}>{resultado.puntaje}/100</Text>
           <Text style={styles.resultSubtitle}>Nivel de Endeudamiento: {resultado.nivel_endeudamiento}%</Text>
        </View>

        <View style={{ marginTop: 5 }}>
           <View style={{ flexDirection: 'row', alignItems: 'flex-end', marginBottom: 10 }}>
              <Image source={require('../../assets/merx_bot.png')} style={{ width: 60, height: 60, borderRadius: 30, borderWidth: 2, borderColor: healthColor, marginRight: 15 }} />
              <View style={{ backgroundColor: colors.card, padding: 10, paddingHorizontal: 15, borderRadius: 20, borderBottomLeftRadius: 0, borderWidth: 1, borderColor: colors.border, shadowColor: healthColor, shadowOpacity: 0.2, shadowRadius: 5, elevation: 3, marginBottom: 5 }}>
                 <Text style={{ color: healthColor, fontWeight: 'bold', fontSize: 14, textTransform: 'uppercase', letterSpacing: 1 }}>Merx Bot</Text>
              </View>
           </View>
           
           <View style={{ backgroundColor: colors.card, padding: 25, borderRadius: 20, borderTopLeftRadius: 0, borderWidth: 1, borderColor: colors.border, shadowColor: healthColor, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4, marginLeft: 20 }}>
             {!resultado.mensaje_ia ? (
               <Text style={styles.aiText}>Analizando tu salud financiera...</Text>
             ) : (
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{flex: 1}}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Diagnóstico 360°</Text>
        </View>
